...
""".strip(),
    ...
    if mirror is None:
        env.pop("BLUEARCH_DIST_BASE_URL", None)
    else:
        env["BLUEARCH_DIST_BASE_URL"] = mirror
...
def test_installer_defaults_to_github_release_downloads(
    ...
    assert urls == [
        f"https://github.com/bluearchio/bluearch-aws-tags/{release_path}/{ASSET}"
    ]


@pytest.mark.parametrize("version", ["1.2", "v1.2.3-rc1", "../latest"])
def test_installer_rejects_noncanonical_release_versions(
    tmp_path: Path,
    version: str,
) -> None:
    result, urls = _probe_first_download_url(tmp_path, version)
...
def test_installer_uses_explicit_mirror_override_with_normalized_tag(
    tmp_path: Path,
) -> None:
    ...
    assert result.returncode != 0
    assert urls == [
        f"https://mirror.example/base/releases/bluearch-aws-tags/v1.2.3/{ASSET}"
    ]
...
def test_installer_accepts_existing_exact_public_core_at_minimum_version(tmp_path: Path) -> None:
    ...
    assert result.returncode == 0, result.stdout + result.stderr
    assert "Using existing bluearch-aws-core >= 0.2.9" in result.stdout
...
def test_installer_replaces_outdated_exact_public_core(tmp_path: Path) -> None:
    ...
    assert "0.2.9" in version.stdout
...
def test_installer_rejects_public_named_core_with_legacy_version_identity(tmp_path: Path) -> None:
    ...
    assert version.stdout.strip() == "bluearch-aws-core 0.2.9"
